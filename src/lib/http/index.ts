...
export const apiClient = new ApiClient({
  baseURL: process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://192.168.1.8:8080",
  timeout: 30_000,
});

// user-svc — auth, user management, project CRUD (:8081)
export const userSvcClient = new ApiClient({
  baseURL: process.env.NEXT_PUBLIC_USER_SVC_URL ?? "http:///192.168.1.8:8081",
  timeout: 15_000,
});

// asset-svc — survey/asset CRUD, analytics triggers (:8082)
export const assetSvcClient = new ApiClient({
  baseURL: process.env.NEXT_PUBLIC_ASSET_SVC_URL ?? "http:///192.168.1.8:8082",
  timeout: 30_000,
});
